...
class AlbumMiddleware(BaseMiddleware):
    def __init__(self, latency: Union[int, float] = 0.1, admin_ids: list=None):
...
#         # Sort the album messages by message_id and add to data
        ...
        has_caption = any(msg.caption for msg in album_messages)
        user_id = event.from_user.id

        if not has_caption and user_id not in self.admin_ids:
            # Удаляем сообщения альбома и отправляем уведомление
